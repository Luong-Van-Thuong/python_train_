...
THAY ĐỔI MỚI:
  1. Tạo model_cfg.yaml NGAY KHI BẮT ĐẦU train (không đợi train xong) -> sập giữa chừng
     vẫn có file để predict.
  2. torch.load(..., weights_only=False) -> resume không bị PyTorch mới chặn.
  3. Kiểm tra class_weights khớp num_classes -> tránh crash khi đổi dataset.
"""
...
def main():
    ...
    class_weights = torch.tensor([0.2, 2.0, 2.0, 1.5, 2.0], dtype=torch.float32)
    # An toàn: weight phải khớp số lớp, tránh crash khi đổi dataset
    if class_weights.numel() != num_classes:
        raise ValueError(
            f"class_weights có {class_weights.numel()} phần tử nhưng num_classes={num_classes}. "
            f"Hãy sửa lại danh sách class_weights cho khớp."
        )
    ...
    # =========================================================
    # GHI model_cfg.yaml NGAY TỪ ĐẦU (không đợi train xong)
    # -> dù sập giữa chừng vẫn có file để chạy predict
    # =========================================================
    cfg = {"arch": args.arch, "encoder": args.encoder, "num_classes": num_classes,
           "names": names, "mean": mean, "std": std, "tile": ds.get("tile")}
    cfg_path = out_dir / "model_cfg.yaml"
    with open(cfg_path, "w", encoding="utf-8") as f:
        yaml.safe_dump(cfg, f, allow_unicode=True, sort_keys=False)
    print(f"[INFO] Đã ghi config sớm: {cfg_path}")
    ...
    if args.resume:
        if last_checkpoint_path.exists():
            ...
            # weights_only=False: cho phép nạp lại cả optimizer/scheduler/scaler state
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Ghi an toàn: lưu file tạm rồi đổi tên -> sập đúng lúc đang ghi cũng không hỏng last.pt
        tmp_path = weights_dir / "last.pt.tmp"
        torch.save(checkpoint_data, tmp_path)
        tmp_path.replace(last_checkpoint_path)
    ...
    # Ghi lại cfg lần cuối (đề phòng có gì thay đổi) - thường giống bản ghi sớm
    with open(cfg_path, "w", encoding="utf-8") as f:
        yaml.safe_dump(cfg, f, allow_unicode=True, sort_keys=False)
