...
def _object_stats(pred, gt, num_classes):
    """Đếm ở mức 'CỤC LỖI' (blob) cho 1 ảnh, gộp mọi lớp lỗi (bỏ nền=0).
    - TP object: 1 cục lỗi THẬT được model chạm >=1 pixel (bắt được).
    - FN object: 1 cục lỗi thật KHÔNG bị chạm (bỏ sót) <- kẻ thù tệ nhất.
    - FP object: 1 cục model VẼ ra ở chỗ không có lỗi thật (báo nhầm).
    Trả (tp, fp, fn). Khác IoU per-pixel: chỉ hỏi 'có trúng cục không', không đếm % trùng khít.
    """
    tp = fp = fn = 0
    for c in range(1, num_classes):
        ...
        for g in range(1, n_gt):                 # từng cục lỗi THẬT
            if pr_c[lbl_gt == g].any():
                tp += 1                          # có chạm -> bắt được
            else:
                fn += 1                          # không chạm -> bỏ sót
        for p in range(1, n_pr):                 # từng cục model VẼ ra
            if not gt_c[lbl_pr == p].any():
                fp += 1                          # không trúng lỗi thật -> báo nhầm
    return tp, fp, fn


@torch.no_grad()
def evaluate(model, loader, device, num_classes):
    """Tính CẢ 2 nhóm thước đo trong 1 lượt duyệt val:
    - Pixel-level: IoU từng lớp + IoU lỗi trung bình (đề cũ).
    - Object-level: Recall / Precision / F1 mức 'cục lỗi' (đề mới cho lỗi nhỏ).
    """
    model.eval()
    tp = fp = fn = tn = None
    o_tp = o_fp = o_fn = 0
...
def main():
    ...
    # --- Công tắc chọn TIÊU CHÍ lưu best.pt. Giữ iou_pixel cho bài toán khác. ---
    ap.add_argument("--best-metric", default="f1_object",
                    choices=["iou_pixel", "recall_object", "f1_object"],
                    help="Chọn best.pt theo: iou_pixel = IoU lỗi trung bình pixel (đề cũ, phạt nặng vật nhỏ). "
                         "recall_object = Recall mức 'cục lỗi' (ít bỏ sót nhất, có thể báo nhầm nhiều). "
                         "f1_object = cân bằng recall+precision mức cục lỗi (KHUYÊN cho lỗi nhỏ).")
    # --- Công tắc chọn "đề thi" (loss). Giữ dice_ce làm chuẩn so sánh (baseline). ---
    ap.add_argument("--loss", default="dice_ce", choices=["dice_ce", "ftl_focal"],
                    help="dice_ce = Dice+CrossEntropy (đề cũ). ftl_focal = FocalTversky+Focal (đề mới cho lỗi nhỏ)")
    ap.add_argument("--tv-alpha", type=float, default=0.3,
                    help="Tversky: mức phạt BÁO NHẦM (FP). Tăng -> em bớt báo nhầm (precision cao hơn)")
    ap.add_argument("--tv-beta", type=float, default=0.7,
                    help="Tversky: mức phạt BỎ SÓT (FN). Tăng -> em bớt bỏ sót lỗi (recall cao hơn)")
    ap.add_argument("--tv-gamma", type=float, default=1.333,
                    help="FocalTversky: mũ 'dồn sức vào ca khó'. 1.0 = Tversky thường; >1 ép luyện mấy cục lỗi bé hay trượt")
    ap.add_argument("--focal-gamma", type=float, default=2.0,
                    help="Focal (thay CrossEntropy): mũ tập trung vào điểm ảnh khó")
    args = ap.parse_args()
    ...
    # Val có thể rỗng (chia_data_unet.py chạy với val-ratio=0). Khi đó bỏ qua đánh giá.
    val_loader = None
    if ds["images"]["val"] and ds["masks"]["val"]:
        ...
        print(f"[INFO] train={len(train_ds)} tile, val=KHÔNG CÓ (chọn best.pt theo train loss)")
    ...
    # --- ĐỘNG CƠ LOSS: CHỌN ĐƯỢC "ĐỀ THI" (ablation) ---
    ...
    defect_classes = list(range(1, num_classes))  # bỏ lớp nền (class 0) khỏi loss vùng

    if args.loss == "dice_ce":
        # ĐỀ CŨ (baseline): phạt BỎ SÓT và BÁO NHẦM ngang nhau.
        ...
        # ĐỀ MỚI (lỗi nhỏ): FocalTversky lệch về "đừng bỏ sót" + Focal cho điểm ảnh khó.
        #   beta > alpha -> phạt BỎ SÓT nặng hơn BÁO NHẦM -> nghiêng về recall.
        #   gamma > 1    -> dồn sức vào mấy cục lỗi bé mà em còn tô trượt.
    ...
    start_epoch = 1
    best_score = -1.0        # dùng khi CÓ val (theo --best-metric, càng cao càng tốt)
    best_loss = float("inf") # dùng khi KHÔNG val (train loss, càng thấp càng tốt)
    ...
    if args.resume:
        if last_checkpoint_path.exists():
            ...
            best_score = checkpoint.get("best_score", checkpoint.get("best_iou", -1.0))
            best_loss = checkpoint.get("best_loss", float("inf"))
            print(f"[SUCCESS] Đồng bộ thành công! Tiếp tục chạy từ Epoch {start_epoch} "
                  f"(best {args.best_metric} cũ = {best_score:.4f})")
        else:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        checkpoint_data = {
            ...
            "best_score": best_score,   # điểm tốt nhất theo --best-metric
            "best_iou": best_score,     # alias tương thích checkpoint cũ
            "best_loss": best_loss
        }
